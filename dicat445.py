...
def simpledickchecker(string,dictu):
    ...
    for i in dictu:
        if i in string:
            ...
            k+=1
            m+=[i]
    if len(m)>2 and (((len(z)*k))/(len(m)*l))>0.22:
...
a='''syssemprogramproblemeesecseedoyouwanssoreporssheproblemnow?'''
b='''syssemprogramproblemeesecseedoyouwanssoreporssheproblemnow?'''[::-1]
dictest(a)
dictest(b)
...
'''
with open('testfo','r') as file2:
    for i in file2:
        dictest(i.lower())
        dictest(i[::-1].lower())
'''
wq='بِسْمِ اللّٰهِ الرَّحْمٰنِ الرَّحِيمِ اللّٰهُمَّ صَلِّ عَلَىٰ مُحَمَّدٍ سَيِّدِ الْمُرْسَلِينَ، وَخَاتَمِ النَّبِيِّينَ، وَحُجَّةِ رَبِّ الْعَالَمِينَ، الْمُنْتَجَبِ فِى الْمِيثَاقِ، الْمُصْطَفىٰ فِى الظِّلالِ، الْمُطَهَّرِ '
def keylist(name):
    d=[]
    with open(name,'r') as file1:
        for line in file1:
...
def numpicker(string):
    ...
    for line in key:
        ...
        print(k.lower())
        print(k.lower()[::-1])
string='Error translating from ArrayBuffer to JSON: SyntaxError: JSON.parse: unexpected character at line 1 column 1 of the JSON data'
print('########')
numpicker(string)
numpicker(string[::-1])