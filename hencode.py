  ...
  for string in neee:
    ...
    for i in m:
       ...
       for lm in fff:
          ...
          if len(lm)%2==0:
            #print(lm)
        #lo=(decodeook(assignit(i)))
        #la=formatter(string,lo)
            #print(lm) 
            a=(decodeook((lm)))
...
string='وا.!چرا یکی باید از حرف طرف مقابل خودشو بزنه ??! اگه منطقی بود که جوابشو می1‌دادی اگه مسخره بود که جواب نداشت'
string='وا.!چرا یکی باید از حرف طرف مقابل خودشو بزنه ??! اگه منطقی بود که جوابشو می1دادی اگه مسخره بود که جواب نداشت'
...
for i in string:
    ...
    x+=1
'''
#def maketextready(string):
    #optionone
    
#its really wierd as i forget where i should not change the ? mark, ! or , that its self in the algo!
#wow
#def makeitready(string):
string='وا.!چرا یکی باید از حرف طرف مقابل خودشو بزنه ??! اگه منطقی بود که جوابشو می1دادی اگه مسخره بود که جواب نداشت '
#def makeitready(string):
#justcheck(string)
#print(formatter('ihdifhsdihfdis?dsd;m;!!/ds.','21122111441111110112224111141120221111'))
